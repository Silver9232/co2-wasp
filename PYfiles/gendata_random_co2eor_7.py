...
output_dir = f"{EOR}_random_seed-{SEED}"
...
for i, (grate, wrate) in enumerate(zip(GRATES, WRATES)):
...
for i in range(mt.floor(len(GRATES) / NPRUNS)):
    ...
    for j in range(NPRUNS):
        ...
        if DELETE == 1:
            os.system(f"rm -rf {output_dir}/{EOR}_{NPRUNS*i+j} {EOR}_{NPRUNS*i+j}.toml")
...
np.save(output_dir+'/'+'grates', GRATES)
np.save(output_dir+'/'+'wrates', WRATES)
